...
class LocalDataSource private constructor(private val mMovieCatalogueDao: MovieCatalogueDao){
    ...
    fun getFavoritedMovies(): DataSource.Factory<Int, MoviesEntity> = mMovieCatalogueDao.getFavoritedMovies()
    fun getFavoritedTvshow(): DataSource.Factory<Int, TvShowEntity> = mMovieCatalogueDao.getFavoritedTvshow()

    fun getMovies(movieId: String): LiveData<MoviesEntity> = mMovieCatalogueDao.getMoviesById(movieId)
    fun getTvShow(tvshowId: String): LiveData<TvShowEntity> = mMovieCatalogueDao.getTvShowById(tvshowId)
